...
// ======= GIVE PLAYERS VOTE ITEM =======

export function giveVoteItem() {
    timeoutId = system.runTimeout(() => {
        DIMENSION.runCommand("replaceitem entity @a[tag=in_game] slot.inventory 0 game:voteManager");
    }, 2400)
}

export function getVoteItemCountdownId() {
    return timeoutId;
}
...
world.afterEvents.itemUse.subscribe((event) => {
    ...
    if (itemStack.typeId == ITEMS.TRIGGER) {
        openMainPanel(source);
    } else if (itemStack.typeId.startsWith(ITEMS.NOTIFICATION_PREFIX)) {
